...
export class PropertyInfo {
  ...
  constructor(name: string, type?: string, docTags?: JSDocTagInfo[]) {
    ...
    this.type = (type ?? 'unknown').replace('[]', '').split('|')[0]?.trim() ?? 'unknown';
  ...
  public static isFloat(docTags: JSDocTagInfo[]): boolean {
    for (const tag of docTags) if (tag.name == 'float') return true;

    return false;
  }

  public static getSampleValue(docTags: JSDocTagInfo[]): string | null {
    for (const tag of docTags) if (tag.name == 'sampleValue') return tag.text?.at(0)?.text ?? null;
