...
export class InfoScanner {
  public static extractInfo(typeChecker: ts.TypeChecker, typeName: string, typeNode: ts.Node) {
    ...
    return type.getProperties().map((symbol): PropertyInfo => {
      ...
      console.log(`    (nullable: ${result.isNullable ? c.green.bold('Y') : c.magenta.bold('N')}, array: ${result.isArray ? c.green.bold('Y') : c.magenta.bold('N')}, with sample value: ${result.hasSampleValue ? c.green.bold('Y') : c.magenta.bold('N')})\n`);
