...
export class SamplerService {
  ...
  private handleNonScalarProperty(property: PropertyInfo, outputSample: OutputSample): void {
    if (this.outputTypes.hasOwnProperty(property.type)) {
      ...
      const value = inputType ? this.generateForComplexType(inputType).get() : null;
      outputSample.add(property, value);
    }
  }

  public castSampleForScalar(property: PropertyInfo): string | number | null {
  ...
  public randomSampleForScalar(scalarType: ScalarTypes): string | number | boolean | null {
    switch (scalarType) {
  ...
  private getInputType(type: string): TypeInfo | null {
    for (const inputType of this.inputTypes) if (inputType.name == type) return inputType;
    return null;
  }
}
