...
export class CliArguments {
  ...
  constructor(
    ...
    if (!fs.existsSync(this.pathOutput)) fs.mkdirSync(this.pathOutput, { recursive: true });
  }

  public static parse(): CliArguments {
    const scriptLocation = path.dirname(path.dirname(process.argv[1] ?? __dirname));
    ...
    const parsedArguments = yargs(hideBin(process.argv))
      ...
      .help()
      .wrap(null).argv as unknown as Record<string, string | undefined>;
